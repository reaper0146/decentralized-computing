...
const createHub = ({
  ...
  const hubAddress = globalHubAddress
    || utils.getContractAddress(
      'hub',
      hubDescMap.hub.contractDesc,
      chainId,
      flavour,
      {
        strict: true,
  ...
  const getContract = (objName) => ({ at } = {}) => {
    try {
      debug(`getContract() ${objName} at`, at);
      const { contractDesc } = hubDescMap[objName];
      const atAddress = at
        || utils.getContractAddress(objName, contractDesc, chainId, flavour, {
          strict: false,
        });
  ...
  const getIExecContract = ({ at = hubAddress } = {}) => getContract('hub')({ at });
  ...
  const fetchContractAddress = (objName) => async ({
    ...
    const { contractDesc, hubPropName } = hubDescMap[objName];
    ...
    const defaultHubAddress = utils.getContractAddress(
      'hub',
      hubDescMap.hub.contractDesc,
      chainId,
      flavour,
    ...
    const fetchedContractAddress = await getHubContract({ at: hub })[
      hubPropName
    ]();
  ...
  const fetchWorkerpoolRegistryAddress = fetchContractAddress(
    'workerpoolRegistry',
  );

  const checkDeployedObj = (objName) => async (
    address,
    { strict = true, hub = hubAddress } = {},
  ) => {
    const hubContract = getHubContract({ at: hub });
  ...
  const createObj = (objName) => async (obj, { hub = hubAddress } = {}) => {
    try {
      debug('hub', hub);
      if (!hub) {
        throw Error(
  ...
  const getObjProps = (objName) => async (at) => {
    try {
      ...
      const objProps = objPropsRPC.reduce(
        (accu, curr, i) => Object.assign(accu, {
          [objPropNames[i]]: curr,
        }),
        {},
      );
  ...
  const getUserObjCount = (objName) => async (
    userAddress,
    { hub = hubAddress } = {},
  ) => {
    try {
      ...
      const hubContract = getHubContract({ at: hub });
      const registryAddress = await hubContract[
        hubDescMap[hubDescMap[objName].registryName].hubPropName
      ]();
      debug('registryAddress', registryAddress);
      const registryContract = getContract(hubDescMap[objName].registryName)({
        at: registryAddress,
  ...
  const createCategory = async (category, { hub = hubAddress } = {}) => {
    try {
      ...
      const tx = await hubContract.createCategory(...args, { gasPrice });
      const txReceipt = await tx.wait(confirms);
      return txReceipt;
    } catch (error) {
  ...
  const getUserObjIdByIndex = (objName) => async (
    userAddress,
    index,
    { hub = hubAddress } = {},
  ) => {
