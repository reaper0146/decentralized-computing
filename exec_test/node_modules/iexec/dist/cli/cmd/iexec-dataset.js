...
cli.name('iexec dataset').usage('<command> [options]').storeOptionsAsProperties(false);
const defaultKeyFileName = 'dataset.key';
const init = cli.command('init');
addGlobalOptions(init);
addWalletLoadOptions(init);
init.option(...option.initDatasetFolders()).option(...option.initTee()).option(...option.datasetKeystoredir()).option(...option.originalDatasetDir()).option(...option.encryptedDatasetDir()).description(desc.initObj(objName)).action(async opts => {
  ...
  try {
    ...
    if (opts.encrypted || opts.tee) {
      ...
      await Promise.all([fs.ensureDir(datasetSecretsFolderPath), fs.ensureDir(originalDatasetFolderPath), fs.ensureDir(encryptedDatasetFolderPath)]);
...
deploy.option(...option.chain()).option(...option.txGasPrice()).option(...option.txConfirms()).description(desc.deployObj(objName)).action(async opts => {
  ...
  try {
    ...
    const {
      address,
      txHash
    } = await deployDataset(chain.contracts, iexecConf[objName]);
...
show.option(...option.chain()).option(...option.user()).description(desc.showObj(objName)).action(async (cliAddressOrIndex, opts) => {
  ...
  try {
    ...
    if (isAddress) {
      ...
      res = await showUserDataset(chain.contracts, addressOrIndex, userAddress);
    }
...
encryptDataset.option(...option.force()).option(...option.datasetKeystoredir()).option(...option.originalDatasetDir()).option(...option.encryptedDatasetDir()).description(desc.encryptDataset()).action(async opts => {
  await checkUpdate(opts);
  const spinner = Spinner(opts);
  ...
  try {
    ...
    const [eDSF, eODF, eEDF] = await Promise.all([fs.pathExists(datasetSecretsFolderPath), fs.pathExists(originalDatasetFolderPath), fs.pathExists(encryptedDatasetFolderPath)]);

    if (!eDSF || !eODF || !eEDF) {
      throw Error('Folders for dataset encryption are missing, did you forget to run "iexec dataset init --encrypted"?');
    }

    const isDatasetFolderEmpty = await isEmptyDir(originalDatasetFolderPath);

    if (isDatasetFolderEmpty) {
      throw Error(`Input folder "${originalDatasetFolderPath}" is empty, nothing to encrypt`);
    }

    const encryptDatasetFile = async datasetFileName => {
      spinner.info(`Encrypting ${datasetFileName}`);
      const key = generateAes256Key();
      const originalFilePath = path.join(originalDatasetFolderPath, datasetFileName);
      ...
      const keyFilePath = await saveTextToFile(`${datasetFileName}.key`, key, {
      ...
      spinner.info(`Generated dataset encryption key for ${datasetFileName} in ${keyFilePath}`);
      const encryptedFilePath = await saveToFile(`${datasetFileName}.enc`, encryptedFileBuffer, {
        fileDir: encryptedDatasetFolderPath,
        force: opts.force
      });
      spinner.info(`Generated encrypted dataset for ${datasetFileName} in ${encryptedFilePath}`);
    ...
    const recursiveEncryptDatasets = async (filesNames, index = 0, encryptedFiles = []) => {
      if (index >= filesNames.length) return encryptedFiles;
      const stats = await fs.lstat(path.join(originalDatasetFolderPath, filesNames[index]));

      if (!stats.isFile()) {
        spinner.info(`Datasets must be single file, skipping ${filesNames[index]}\n`);
      } else {
...
pushSecret.option(...option.chain()).option(...option.secretPath()).description(desc.pushDatasetSecret()).action(async (objAddress, opts) => {
  await checkUpdate(opts);
  const spinner = Spinner(opts);
  ...
  try {
    ...
    const resourceAddress = objAddress || (await loadDeployedObj(objName).then(deployedObj => deployedObj && deployedObj[chain.id]));
    debug('resourceAddress', resourceAddress);

    if (!resourceAddress) {
      throw Error('Missing datasetAddress argument and no dataset found in "deployed.json"');
    }
    ...
    if (opts.secretPath) {
      ...
      secretFilePath = path.join(datasetSecretsFolderPath, defaultKeyFileName);
      spinner.info(`No --secret-path <path> option, using default ${secretFilePath}`);
    }

    const secretToPush = (await fs.readFile(secretFilePath, 'utf8')).trim();
    debug('secretToPush', secretToPush);
    await connectKeystore(chain, keystore);
    const isPushed = await secretMgtServ.pushWeb3Secret(contracts, sms, resourceAddress, secretToPush);
...
checkSecret.option(...option.chain()).description(desc.checkSecret()).action(async (objAddress, opts) => {
  await checkUpdate(opts);
  const spinner = Spinner(opts);
  ...
  try {
    ...
    const sms = getPropertyFormChain(chain, 'sms');
    const secretIsSet = await secretMgtServ.checkWeb3SecretExists(chain.contracts, sms, resourceAddress);
...
publish.description(desc.publishObj(objName)).option(...option.chain()).option(...option.force()).option(...orderOption.price()).option(...orderOption.volume()).option(...orderOption.tag()).option(...orderOption.apprestrict()).option(...orderOption.workerpoolrestrict()).option(...orderOption.requesterrestrict()).action(async (objAddress, opts) => {
  await checkUpdate(opts);
  const spinner = Spinner(opts);
  const walletOptions = await computeWalletLoadOptions(opts);
  const keystore = Keystore(walletOptions);
  ...
  try {
    ...
    const orderHash = await publishDatasetorder(chain.contracts, getPropertyFormChain(chain, 'iexecGateway'), signedOrder);
...
unpublish.description(desc.unpublishObj(objName)).option(...option.chain()).option(...option.force()).option(...option.unpublishAllOrders()).action(async (objAddress, opts) => {
  ...
  try {
    ...
    await connectKeystore(chain, keystore);
    const unpublished = all ? await unpublishAllDatasetorders(chain.contracts, getPropertyFormChain(chain, 'iexecGateway'), address) : await unpublishLastDatasetorder(chain.contracts, getPropertyFormChain(chain, 'iexecGateway'), address);
