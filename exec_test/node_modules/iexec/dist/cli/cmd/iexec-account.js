...
deposit.option(...option.chain()).option(...option.txGasPrice()).option(...option.txConfirms()).description(desc.deposit()).action(async (amount, unit, opts) => {
...
withdraw.option(...option.chain()).option(...option.txGasPrice()).option(...option.txConfirms()).description(desc.withdraw()).action(async (amount, unit, opts) => {
  await checkUpdate(opts);
  const spinner = Spinner(opts);
...
show.option(...option.chain()).description(desc.showObj('iExec', objName)).action(async (address, opts) => {
  await checkUpdate(opts);
  const spinner = Spinner(opts);
  ...
  try {
    ...
    spinner.succeed(`Account balances (RLC):${pretty({
