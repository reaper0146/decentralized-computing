...
show.option(...option.chain()).option(...option.watch()).option(...option.download()).option(...option.decrypt()).option(...option.beneficiaryKeystoredir()).option(...option.beneficiaryKeyFile()).description(desc.showObj(objName)).action(async (taskid, opts) => {
  await checkUpdate(opts);
  const spinner = Spinner(opts);

  try {
    const walletOptions = await computeWalletLoadOptions(opts);
    const keystore = Keystore(Object.assign(walletOptions, !opts.download && {
      isSigner: false
    }));
    ...
    if (opts.download) {
      if (taskResult.status === 3) {
        ...
        resultPath = path.join(process.cwd(), resultFileName.length > 4 && resultFileName.substr(resultFileName.length - 4) === '.zip' ? resultFileName : `${resultFileName}.zip`);
        ...
        if (opts.decrypt) {
          ...
          try {
            ...
            debug(error);
            throw Error(`Failed to load beneficiary key from "${beneficiaryKeyPath}"`);
          }

          const result = await decryptResult(await res.arrayBuffer(), beneficiaryKey);
        ...
        spinner.info(`Task status is not COMPLETED, option ${option.download()[0]} will be ignored`);
      }
    }
    ...
    if (claimable) {
      spinner.info('Consensus deadline reached before consensus. You can claim the task to get a full refund using "iexec task claim"');
    }
  } catch (error) {
...
claim.option(...option.chain()).option(...option.txGasPrice()).option(...option.txConfirms()).description(desc.claimObj(objName)).action(async (taskid, opts) => {
  await checkUpdate(opts);
  const spinner = Spinner(opts);
  ...
  try {
    ...
    spinner.succeed('Task successfully claimed', {
      raw: {
        txHash
      }
    });
