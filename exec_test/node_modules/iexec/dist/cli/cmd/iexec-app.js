...
const {
  checkBalance
} = require('../../common/modules/account');
...
const objName = 'app';
cli.name('iexec app').usage('<command> [options]').storeOptionsAsProperties(false);
...
deploy.option(...option.chain()).option(...option.txGasPrice()).option(...option.txConfirms()).description(desc.deployObj(objName)).action(async opts => {
  ...
  try {
    ...
    const {
      address,
      txHash
    } = await deployApp(chain.contracts, iexecConf[objName]);
...
publish.description(desc.publishObj(objName)).option(...option.chain()).option(...option.force()).option(...orderOption.price()).option(...orderOption.volume()).option(...orderOption.tag()).option(...orderOption.datasetrestrict()).option(...orderOption.workerpoolrestrict()).option(...orderOption.requesterrestrict()).action(async (objAddress, opts) => {
  await checkUpdate(opts);
  const spinner = Spinner(opts);
  const walletOptions = await computeWalletLoadOptions(opts);
  const keystore = Keystore(walletOptions);
  ...
  try {
    ...
    const orderHash = await publishApporder(chain.contracts, getPropertyFormChain(chain, 'iexecGateway'), signedOrder);
...
unpublish.description(desc.unpublishObj(objName)).option(...option.chain()).option(...option.force()).option(...option.unpublishAllOrders()).action(async (objAddress, opts) => {
  ...
  try {
    ...
    await connectKeystore(chain, keystore);
    const unpublished = all ? await unpublishAllApporders(chain.contracts, getPropertyFormChain(chain, 'iexecGateway'), address) : await unpublishLastApporder(chain.contracts, getPropertyFormChain(chain, 'iexecGateway'), address);
...
run.option(...option.chain()).option(...option.txGasPrice()).option(...option.txConfirms()).option(...option.force()).option(...option.appRunWatch()).option(...orderOption.dataset()).option(...orderOption.workerpool()).option(...orderOption.requestArgs()).option(...orderOption.requestInputFiles()).option(...orderOption.category()).option(...orderOption.tag()).option(...orderOption.requestStorageProvider()).option(...orderOption.callback()).option(...orderOption.requestEncryptResult()).option(...orderOption.trust()).option(...orderOption.beneficiary()).option(...orderOption.params()).option(...option.skipRequestCheck()).description(desc.appRun()).action(async (appAddress, opts) => {
  await checkUpdate(opts);
  const spinner = Spinner(opts);
  ...
  try {
    ...
    const app = appAddress || (await loadDeployedObj('app').then(deployedApp => deployedApp && deployedApp[chain.id]));

    if (!app) {
      throw Error(`Missing appAddress and no app found in "deployed.json" for chain ${chain.id}`);
    }

    debug('useDeployedApp', useDeployedApp, 'app', app);

    if (useDeployedApp) {
      spinner.info('No app specified, using last app deployed from "deployed.json"');
    }

    const useDataset = opts.dataset !== undefined;
    const useDeployedDataset = useDataset && opts.dataset === 'deployed';
    const dataset = useDataset && (useDeployedDataset ? await loadDeployedObj('dataset').then(deployedDataset => deployedDataset && deployedDataset[chain.id]) : opts.dataset);

    if (useDataset && !dataset) {
      throw Error(`No dataset found in "deployed.json" for chain ${chain.id}`);
    }

    debug('useDataset', useDataset, 'useDeployedDataset', useDeployedDataset, 'dataset', dataset);
    ...
    const workerpool = runOnWorkerpool && (useDeployedWorkerpool ? await loadDeployedObj('workerpool').then(deployedWorkerpool => deployedWorkerpool && deployedWorkerpool[chain.id]) : opts.workerpool);

    if (runOnWorkerpool && !workerpool) {
      throw Error(`No workerpool found in "deployed.json" for chain ${chain.id}`);
    }

    debug('runOnWorkerpool', runOnWorkerpool, 'useDeployedWorkerpool', useDeployedWorkerpool, 'workerpool', workerpool);
    ...
    const inputParamsInputFiles = await paramsInputFilesArraySchema().validate(opts.inputFiles);
    const inputParamsStorageProvider = await paramsStorageProviderSchema().validate(opts.storageProvider);
    const inputParamsResultEncrytion = await paramsEncryptResultSchema().validate(opts.encryptResult);
    const params = { ...(inputParams !== undefined && JSON.parse(inputParams)),
    ...
    const beneficiary = opts.beneficiary === undefined ? undefined : await addressSchema({
      ethProvider: chain.contracts.provider
    ...
    const getApporder = async () => {
      ...
      const {
        orders
      } = await fetchAppOrderbook(chain.contracts, getPropertyFormChain(chain, 'iexecGateway'), app, {
        requester,
        ...(useDataset && {
          dataset
        }),
    ...
    const getDatasetorder = async () => {
      spinner.info(useDataset ? `Using dataset ${dataset}` : 'Not using dataset');
      if (!useDataset) return NULL_DATASETORDER;
      if (typeof dataset === 'string' && dataset.toLowerCase() === NULL_ADDRESS) return NULL_DATASETORDER;
      ...
      const {
        orders
      } = await fetchDatasetOrderbook(chain.contracts, getPropertyFormChain(chain, 'iexecGateway'), dataset, {
        app,
        requester,
        ...(runOnWorkerpool && {
          workerpool
    ...
    const getWorkerpoolorder = async () => {
      spinner.info(workerpool ? `Using workerpool ${workerpool}` : 'Using any workerpool');
      ...
      if (runOnWorkerpool) {
        ...
        const workerpoolOwner = await getWorkerpoolOwner(chain.contracts, workerpool);
      ...
      const fetchWorkerpoolOrder = async (catid = 0, {
        strict = false
      } = {}) => {
        debug('try category', catid, 'strict', strict);
        const {
          orders
        } = await fetchWorkerpoolOrderbook(chain.contracts, getPropertyFormChain(chain, 'iexecGateway'), {
          category: catid,
          app,
          ...(useDataset && {
            dataset
        ...
        const getFirstOpen = async (i = 0) => {
          ...
          if (order) {
            const workerpoolVolume = await getRemainingVolume(chain.contracts, WORKERPOOL_ORDER, order);
        ...
        if (strict) {
          throw Error(`No workerpoolorder matching your conditions available in category ${category}`);
        }
        ...
        try {
          ...
          debug(error);
          throw Error('No workerpoolorder matching your conditions currently available');
        }
    ...
    const requestorder = await signRequestorder(chain.contracts, requestorderToSign);
    debug('requestorder', requestorder);
    const totalCost = new BN(requestorder.appmaxprice).add(new BN(requestorder.datasetmaxprice)).add(new BN(requestorder.workerpoolmaxprice)).mul(new BN(requestorder.volume));
    const {
      stake
    } = await checkBalance(chain.contracts, requester);

    if (totalCost.gt(stake)) {
      throw Error(`Not enough RLC on your account (${formatRLC(totalCost)} RLC required). Run "iexec account deposit" to topup your account.`);
    }

    spinner.stop();

    if (!opts.force) {
      await prompt.custom(`Do you want to spend ${formatRLC(totalCost)} RLC to execute the following request: ${pretty({
        app: `${requestorder.app} (${formatRLC(requestorder.appmaxprice)} RLC)`,
        dataset: requestorder.dataset !== NULL_ADDRESS ? `${requestorder.dataset} (${formatRLC(requestorder.datasetmaxprice)} RLC)` : undefined,
        workerpool: `${requestorder.workerpool} (${formatRLC(requestorder.workerpoolmaxprice)} RLC)`,
        params: requestorder.params && JSON.parse(requestorder.params) || undefined,
        category: requestorder.category,
        tag: requestorder.tag !== NULL_BYTES32 ? requestorder.tag : undefined,
        callback: requestorder.callback !== NULL_ADDRESS ? requestorder.callback : undefined,
        beneficiary: requestorder.beneficiary !== requestorder.requester ? requestorder.beneficiary : undefined
      })}`);
    }
    ...
    const {
      ...
      volume,
      txHash
    } = await matchOrders(chain.contracts, apporder, datasetorder, workerpoolorder, requestorder);
    result.deals.push({
      dealid,
    ...
    if (!watch) {
      ...
      const tasksArray = Object.values(dealFinalState.tasks).map(stringifyNestedBn);
      result.tasks = tasksArray;
      const failedTasks = tasksArray.reduce((acc, curr) => curr.status !== 3 ? [...acc, curr] : acc, []);

      if (failedTasks.length === 0) {
        spinner.succeed(`App run successful:\n${renderTasksStatus(dealFinalState.tasks, {
          detailed: true
        })}`, {
          raw: result
        });
...
requestRun.option(...option.chain()).option(...option.force()).option(...orderOption.dataset({
  allowDeployed: false
})).option(...orderOption.workerpool({
  allowDeployed: false
})).option(...orderOption.appprice()).option(...orderOption.datasetprice()).option(...orderOption.workerpoolprice()).option(...orderOption.requestArgs()).option(...orderOption.requestInputFiles()).option(...orderOption.category()).option(...orderOption.tag()).option(...orderOption.volume()).option(...orderOption.requestStorageProvider()).option(...orderOption.callback()).option(...orderOption.requestEncryptResult()).option(...orderOption.trust()).option(...orderOption.beneficiary()).option(...orderOption.params()).option(...option.skipRequestCheck()).description(desc.requestRun()).action(async (app, opts) => {
  await checkUpdate(opts);
  const spinner = Spinner(opts);
  ...
  try {
    ...
    if (dataset !== NULL_ADDRESS && !(await checkDeployedDataset(chain.contracts, dataset))) {
    ...
    if (workerpool !== NULL_ADDRESS && !(await checkDeployedWorkerpool(chain.contracts, workerpool))) {
    ...
    const datasetprice = await nRlcAmountSchema().validate(dataset !== NULL_ADDRESS && opts.datasetPrice || 0);
    debug('datasetprice', datasetprice);
    const workerpoolprice = await nRlcAmountSchema().validate(opts.workerpoolPrice || 0);
    ...
    if (!opts.force) {
      await prompt.custom(`Do you want to place the following execution request for ${formatRLC(totalCost)} RLC \n - The amount will be taken from your account only if your request is filled.\n - Your request will be disabled if your stake drops under ${formatRLC(totalCost)} RLC. ${pretty({
        app: `${requestorder.app} (${formatRLC(requestorder.appmaxprice)} RLC)`,
        dataset: requestorder.dataset !== NULL_ADDRESS ? `${requestorder.dataset} (${formatRLC(requestorder.datasetmaxprice)} RLC)` : undefined,
        workerpool: `${requestorder.workerpool} (${formatRLC(requestorder.workerpoolmaxprice)} RLC)`,
    ...
    const orderHash = await publishRequestorder(chain.contracts, getPropertyFormChain(chain, 'iexecGateway'), requestorder);
    spinner.succeed(`Successfully published requestorder with orderHash ${orderHash}\nRun "iexec orderbook requester ${requester}" to show published requestorders`, {
      raw: {
        orderHash
      }
    });
