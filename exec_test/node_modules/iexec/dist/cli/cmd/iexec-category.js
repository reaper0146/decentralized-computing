...
create.option(...option.chain()).option(...option.txGasPrice()).option(...option.txConfirms()).description(desc.createObj(objName)).action(async opts => {
  await checkUpdate(opts);
  const spinner = Spinner(opts);
  ...
  try {
    ...
    const [iexecConf, chain] = await Promise.all([loadIExecConf(), loadChain(opts.chain, {
      txOptions,
      spinner
    })]);
...
show.option(...option.chain()).description(desc.showObj(objName, 'hub')).action(async (index, opts) => {
  await checkUpdate(opts);
  const spinner = Spinner(opts);
  ...
  try {
    ...
    spinner.succeed(`Category at index ${index} details:${pretty(category)}`, {
      raw: {
        index,
        category
      }
...
count.option(...option.chain()).description(desc.showObj(objName, 'hub')).action(async opts => {
  await checkUpdate(opts);
  const spinner = Spinner(opts);
