...
const buyConfSchema = () => object({
  ...
  }).required().noUnknown().strict(),
  trust: uint256Schema(),
  tag: bytes32Schema(),
  callback: addressSchema()
});

const dappSchema = () => baseSchema().shape({
...
const datasetSchema = () => baseSchema().shape({
  license: string().required(),
  author: string().required(),
  categories: string(),
  dataset: object({
...
const workerpoolSchema = () => baseSchema().shape({
  workerpool: object({
    owner: addressSchema().required(),
...
validate.description(desc.validateRessource()).action(async (objName, opts) => {
  ...
  try {
    if (!objectNames.includes(objName)) {
      throw Error(`Unknown object "${objName}". Must be one of [${objectNames}]`);
    }

    const objectName = objectMap[objName].name; // validate iexec.json
    ...
    try {
      await objectMap[objName].validationSchema().validate(iexecConf, {
        strict: true
      });
    ...
    try {
      ...
      if (!(logoSize.width === LOGO_SIDE && logoSize.height === LOGO_SIDE)) {
        throw Error(`${iexecConf.logo} dimensions should be ${LOGO_SIDE}px by ${LOGO_SIDE}px, NOT ${logoSize.width} by ${logoSize.height}`);
      }
      ...
      const errorMessage = logoError.code === 'ENOENT' ? `Missing "${iexecConf.logo}" logo image file` : logoError.message;
    ...
    try {
      ...
      if (!(objectName in deployedObj)) {
        throw Error(`Missing ${objectName} field. You should run "iexec ${objName} deploy"`);
      }
    ...
    if (failed.length === 0) {
      spinner.succeed(`${objName} description is valid. You can now submit it to the ${objName} registry: ${objectMap[objName].registry}`, {
        raw: {
          validated
        }
      });
