...
const decrypt = async (encryptedJSON, password) => {
  try {
    const {
      privateKey
    } = await Wallet.fromEncryptedJson(JSON.stringify(encryptedJSON), password);
    const {
      wallet
    } = walletFromPrivKey(privateKey);
    return wallet;
  } catch (error) {
    debug('decrypt()', error);
    throw error;
...
const saveWallet = async (userWallet, options) => {
  ...
  if (options.walletOptions && options.walletOptions.global) {
    const keystoredir = osDefaultPathMap[os.platform()] ? osDefaultPathMap[os.platform()].keystoredir : osDefaultPathMap.fallback.keystoredir;
  ...
  if (options.walletOptions && options.walletOptions.password) {
    const encryptedWallet = await encrypt(userWallet.privateKey, options.walletOptions.password); // Wallet name
    ...
    const creationDate = `${now.getUTCFullYear()}-${pad(now.getUTCMonth() + 1)}-${pad(now.getUTCDate())}T${pad(now.getUTCHours())}-${pad(now.getUTCMinutes())}-${pad(now.getUTCSeconds())}.${(now.getUTCMilliseconds() / 1000).toFixed(3).slice(2, 5)}000000Z`;
    const walletName = `UTC--${creationDate}--${userWallet.address.slice(2)}`;
    const fileName = await saveEncryptedWalletConf(encryptedWallet, { ...options,
      walletName,
      fileDir
    });
    return {
...
const Keystore = ({
  ...
  let cachedWallet = {};
  let password = walletOptions && walletOptions.password || false; // keystoreDir
  ...
  const getMostRecentWalletFileName = async () => {
    ...
    const sortedWallet = files.filter(e => e.split('--')[2]).sort(descSortWallet);
  ...
  const getWalletFileName = async () => {
    ...
    if (walletOptions && walletOptions.walletAddress) {
      ...
      try {
        ...
        throw Error(`Missing keystore directory ${fileDir}, did you forget to run "iexec wallet create" ?`);
      }

      const match = files.filter(e => {
        const address = e.split('--')[2];
        return address && ('0x'.concat(address).toLowerCase() === walletOptions.walletAddress.toLowerCase() || address.toLowerCase() === walletOptions.walletAddress.toLowerCase());
      ...
      throw Error(`No wallet file matching address ${walletOptions.walletAddress} found in ${fileDir}`);
    }

    const existsUnencrypted = await fs.existsSync(path.join(process.cwd(), 'wallet.json'));
  ...
  const load = async () => {
    ...
    const fileName = await getWalletFileName(); // try local unencrypted
    ...
    if (!fileName) {
      try {
        ...
        const {
          privateKey
        } = await loadWalletConf(loadingOptions);
        pk = privateKey;
      } catch (error) {
        debug('try load unencrypted', error);
        throw Error(`Missing option ${option.walletAddress()[0]} or ${option.walletFileName()[0]} and no wallet.json found in working directory`);
      }
    } // try encrypted
    ...
    if (!pk) {
      try {
        ...
        if (!password) {
          password = await prompt.password(`Using wallet ${fileName}\nPlease enter your password to unlock your wallet`);
        }
  ...
  const loadWalletAddress = async () => {
    ...
    let walletAddress; // try local unencrypted
    ...
    if (!walletAddress) {
      try {
        ...
        const {
          address
        } = await loadEncryptedWalletConf(loadingOptions);
        walletAddress = checksummedAddress(address);
      } catch (error) {
