...
const listOfChoices = (arrayOfChoices, init = '') => arrayOfChoices.reduce((acc, curr) => acc ? `${acc}|"${curr}"` : `"${curr}"`, init);
...
const option = {
  ...
  cancelWorkerpoolOrder: () => ['--workerpool', 'cancel a signed workerpoolorder'],
  cancelRequestOrder: () => ['--request', 'cancel a signed userorder'],
  publishAppOrder: () => ['--app', 'publish a signed apporder on iExec marketplace'],
  publishDatasetOrder: () => ['--dataset', 'publish a signed datasetorder on iExec marketplace'],
  publishWorkerpoolOrder: () => ['--workerpool', 'publish a signed workerpoolorder on iExec marketplace'],
  publishRequestOrder: () => ['--request', 'publish a signed requestorder on iExec marketplace'],
  unpublishAppOrder: () => ['--app [orderHash]', 'unpublish a signed apporder from iExec marketplace'],
  unpublishDatasetOrder: () => ['--dataset [orderHash]', 'unpublish a signed datasetorder from iExec marketplace'],
  unpublishWorkerpoolOrder: () => ['--workerpool [orderHash]', 'unpublish a signed workerpoolorder from iExec marketplace'],
  unpublishRequestOrder: () => ['--request [orderHash]', 'unpublish a signed requestorder from iExec marketplace'],
  ...
  showWorkerpoolOrder: () => ['--workerpool [orderHash]', 'show a workerpoolorder'],
  showRequestOrder: () => ['--request [orderHash]', 'show a requestorder'],
  showOrderDeals: () => ['--deals', 'show the deals produced by the order'],
  fillAppOrder: () => ['--app <orderHash>', 'specify the app order from the marketplace to fill'],
  fillDatasetOrder: () => ['--dataset <orderHash>', 'specify the dataset order from the marketplace to fill'],
  fillWorkerpoolOrder: () => ['--workerpool <orderHash>', 'specify the wokerpool order from the marketplace to fill'],
  fillRequestOrder: () => ['--request <orderHash>', 'specify the requestorder from the marketplace to fill'],
  fillRequestParams: () => ['--params <json>', 'specify the params of the request, existing request order will be ignored\n* usage: --params \'{"iexec_args":"dostuff","iexec_input_files":["https://example.com/file.zip"]}\''],
  ...
  forceCreate: () => ['--force', 'force wallet creation even if old wallet exists'],
  force: () => ['--force', 'force perform action without prompting user'],
  showPrivateKey: () => ['--show-private-key', 'allow displaying walletprivate key'],
  watch: () => ['--watch', 'watch execution status changes'],
  download: () => ['--download [fileName]', 'download a task result data to local filesystem, if completed'],
  ...
  filterBeneficiarySpecific: () => ['--beneficiary <address>', 'filter by beneficiary'],
  includeAppSpecific: () => ['--app <address>', 'include private orders for specified app'],
  includeDatasetSpecific: () => ['--dataset <address>', 'include private orders for specified dataset'],
  includeWorkerpoolSpecific: () => ['--workerpool <address>', 'include private orders for specified workerpool'],
  includeRequesterSpecific: () => ['--requester <address>', 'include private orders for specified requester'],
  requiredTag: () => ['--require-tag <tag>', 'specify minimum required tags\n* usage: --require-tag tag1,tag2'],
  maxTag: () => ['--max-tag <tag>', 'specify maximun tags (exclude not listed tags)\n* usage: --max-tag tag1,tag2'],
  ...
  password: () => ['--password <password>', 'password used to encrypt the wallet (unsafe)'],
  unencrypted: () => ['--unencrypted', 'generate unsafe unencrypted wallet in working directory (--keystoredir option is ignored)'],
  keystoredir: () => ['--keystoredir <path>', `specify the wallet directory <${listOfChoices(['global', 'local'])}|custom>`],
  walletAddress: () => ['--wallet-address <walletAddress>', 'specify the address of the wallet to use'],
  walletFileName: () => ['--wallet-file <walletFileName>', 'specify the name of the wallet file to use'],
  secretPath: () => ['--secret-path <secretPath>', 'push the secret from a file'],
  datasetKeystoredir: () => ['--dataset-keystoredir <path>', 'specify dataset TEE key directory'],
  beneficiaryKeystoredir: () => ['--beneficiary-keystoredir <path>', 'specify beneficiary TEE keys directory'],
  beneficiaryKeyFile: () => ['--beneficiary-key-file <fileName>', 'specify beneficiary TEE key file to use'],
  initDatasetFolders: () => ['--encrypted', 'init datasets folder tree for dataset encryption'],
  encryptedDatasetDir: () => ['--encrypted-dataset-dir <path>', 'specify the encrypted dataset directory'],
  originalDatasetDir: () => ['--original-dataset-dir <path>', 'specify the original dataset directory'],
  txGasPrice: () => ['--gas-price <amount unit...>', 'set custom gas price for transactions (default unit wei)'],
  txConfirms: () => ['--confirms <blockCount>', 'set custom block count to wait for transactions confirmation (default 1 block)'],
  forceUpdateSecret: () => ['--force-update', 'update if already exists'],
  storageToken: () => ['--token <token>', 'storage provider authorization token (unsafe)'],
  skipRequestCheck: () => ['--skip-request-check', 'skip request validity checks, this may result in task execution fail']
};
const orderOption = {
  app: ({
    allowDeployed = true
  } = {}) => [`--app <${allowDeployed ? listOfChoices(['deployed'], 'address') : 'address'}>`, `app address${allowDeployed ? ', use "deployed" to use last deployed from "deployed.json"' : ''}`],
  dataset: ({
    allowDeployed = true
  } = {}) => [`--dataset <${allowDeployed ? listOfChoices(['deployed'], 'address') : 'address'}>`, `dataset address${allowDeployed ? ', use "deployed" to use last deployed from "deployed.json"' : ''}`],
  workerpool: ({
    allowDeployed = true
  } = {}) => [`--workerpool <${allowDeployed ? listOfChoices(['deployed'], 'address') : 'address'}>`, `workerpool address${allowDeployed ? ', use "deployed" to use last deployed from "deployed.json"' : ''}`],
  price: () => ['--price <amount unit...>', 'price per task (default unit nRLC)'],
  appprice: () => ['--app-price <amount unit...>', 'app price per task (default unit nRLC)'],
  datasetprice: () => ['--dataset-price <amount unit...>', 'dataset price per task (default unit nRLC)'],
  workerpoolprice: () => ['--workerpool-price <amount unit...>', 'workerpool price per task (default unit nRLC)'],
  ...
  apprestrict: () => ['--app-restrict <address>', 'restrict usage to specific app'],
  datasetrestrict: () => ['--dataset-restrict <address>', 'restrict usage to specific dataset'],
  workerpoolrestrict: () => ['--workerpool-restrict <address>', 'restrict usage to specific workerpool'],
  requesterrestrict: () => ['--requester-restrict <address>', 'restrict usage to specific requester'],
  beneficiary: () => ['--beneficiary <address>', 'specify the beneficiary of the request (default user address)'],
  callback: () => ['--callback <address>', 'specify the callback address of the request'],
  params: () => ['--params <json>', 'specify the params of the request\n* usage: --params \'{"iexec_args":"dostuff","iexec_input_files":["https://example.com/file.zip"]}\''],
  requestArgs: () => ['--args <string>', 'specify the arguments to pass to the app'],
  requestInputFiles: () => ['--input-files <fileUrl>', 'specify the URL of input files to be used by the app\n* usage: --input-files https://example.com/foo.txt,https://example.com/bar.zip'],
  requestEncryptResult: () => ['--encrypt-result', 'encrypt the result archive with the beneficiary public key'],
  requestStorageProvider: () => [`--storage-provider <${listOfChoices(storageProviders())}>`, 'specify the storage to use to store the result archive']
};
...
const promptConfirmedPassword = async (message, confirmation = 'Please confirm your password') => {
  const pw1 = await promptPassword(message, {
    strict: false
  });
...
const prompt = {
  ...
  dirNotEmpty: (dir, options) => question(`Directory ${dir} is not empty, continue and replace content?`, options),
  fileExists: (filePath, options) => question(`File ${filePath} already exists, continue and replace?`, options),
  transfer: (currency, amount, chainName, to, chainId) => question(`Do you want to send ${amount} ${chainName} ${currency} to ${to} [chainId: ${chainId}]`),
  wrap: (amount, chainId) => question(`Do you want to swap ${amount} RLC for eRLC (1 RLC = 1 eRLC) [chainId: ${chainId}]`),
  unwrap: (amount, chainId) => question(`Do you want to swap ${amount} eRLC for RLC (1 eRLC = 1 RLC) [chainId: ${chainId}]`),
  ...
  unpublishOrder: (objName, address, all) => question(`Do you want to unpublish ${all ? 'all your' : 'your last'} ${objName}order${all ? 's' : ''} for ${objName} ${address}?`),
  signGeneratedOrder: (orderName, order) => question(`the following ${orderName} has been created, do you want to sign it and complete your purchase? ${order}`),
  limitedVolume: (available, ask) => question(`Your user order is valid for ${ask} work executions but other orders allow only ${available} work executions, do you want to continue?`),
  unpublishFromJsonFile: (orderName, order) => question(`Do you want to unpublish the following ${orderName}? ${order}`),
  more: () => question('Show more?', {
    rejectDefault: true,
    strict: false
  })
};
...
const checkUpdate = async opts => {
  if (opts && !opts.quiet && !opts.raw) {
    ...
    if (update) {
      ...
      spin.info(`iExec SDK update available ${packageJSON.version} →  ${update.latest}, Run "${upgradeCMD}" to update ("--quiet" or "--raw" disable update notification)\n`);
    }
  }
};
...
const computeWalletCreateOptions = async opts => {
  ...
  try {
    ...
    if (opts.password) {
      pw = opts.password;
      spinner.warn('Option --password may be unsafe, make sure to know what you do');
    } else if (!opts.unencrypted) {
      pw = await prompt.confimedPassword('Please choose a password for wallet encryption');
    }
    ...
    if (opts.unencrypted) {
      spinner.warn('Using --unencrypted will generate unprotected unencrypted wallet, this is unsafe, make sure to know what you do');
    }
    ...
    const keystorePath = opts.keystoredir && opts.keystoredir !== 'local' && opts.keystoredir !== 'global' ? opts.keystoredir : false;
    return {
      walletOptions: {
        global,
        local,
...
const computeWalletLoadOptions = opts => {
  try {
    const global = opts && opts.keystoredir && opts.keystoredir === 'global' || !opts || !opts.keystoredir;
    const local = opts && opts.keystoredir && opts.keystoredir === 'local' || false;
    const keystorePath = opts && opts.keystoredir && opts.keystoredir !== 'local' && opts.keystoredir !== 'global' ? opts.keystoredir : false;
...
const createEncFolderPaths = (opts = {}) => {
  const absolutePath = relativeOrAbsolutePath => path.isAbsolute(relativeOrAbsolutePath) ? relativeOrAbsolutePath : path.join(process.cwd(), relativeOrAbsolutePath);

  const datasetSecretsFolderPath = opts.datasetKeystoredir ? absolutePath(opts.datasetKeystoredir) : path.join(process.cwd(), secretsFolderName, datasetSecretsFolderName);
  const beneficiarySecretsFolderPath = opts.beneficiaryKeystoredir ? absolutePath(opts.beneficiaryKeystoredir) : path.join(process.cwd(), secretsFolderName, beneficiarySecretsFolderName);
  const originalDatasetFolderPath = opts.originalDatasetDir ? absolutePath(opts.originalDatasetDir) : path.join(process.cwd(), datasetsFolderName, originalDatasetFolderName);
  const encryptedDatasetFolderPath = opts.encryptedDatasetDir ? absolutePath(opts.encryptedDatasetDir) : path.join(process.cwd(), datasetsFolderName, encryptedDatasetFolderName);
  const paths = {
...
const computeTxOptions = async opts => {
  ...
  if (opts.gasPrice) {
    ...
    const bnGasPrice = new BN(await weiAmountSchema({
      defaultUnit: 'wei'
    }).label('gas-price').validate(opts.gasPrice));
  ...
  if (opts.confirms !== undefined) {
    ...
    confirms = await positiveStrictIntSchema().label('confirms').validate(opts.confirms, {
...
const getPropertyFormChain = (chain, property, {
  strict = true
} = {}) => {
...
const handleError = (error, cli, opts) => {
  ...
  const commandName = cli._name.split('-').join(' ').concat(' ', lastCommandName);
...
const isEthAddress = (address, {
  ...
  const isEns = typeof address === 'string' && address.substr(address.length - 4, 4) === '.eth';
...
const isBytes32 = (str, {
  ...
  if (typeof str !== 'string' || str.length !== 66 || str.substr(0, 2) !== '0x') {
...
const displayPaginableRequest = async ({
  ...
  emptyResultsMessage,
  createResultsMessage = (callResults, initilResultsCount, totalCount) => `Results (${initilResultsCount + 1} to ${initilResultsCount + callResults.length}${totalCount ? ` of ${totalCount}` : ''}):\n${pretty(callResults)}`,
  spinner,
  raw = false
}, {
  results = [],
...
const renderTasksStatus = (tasksStatusMap, {
  ...
  const runningTasksArray = tasksArray.filter(task => task.status !== 3 && !task.taskTimedOut);
  ...
  const completedMsg = completedTasksArray.length > 0 ? `${completedTasksArray.length}/${tasksArray.length} tasks completed${detailed ? `:${pretty(completedTasksArray.map(({
    idx,
    taskid
  }) => `Task idx ${idx} (${taskid})`))}` : '\n'}` : '';
  const failedMsg = timedoutTasksArray.length > 0 ? `${timedoutTasksArray.length}/${tasksArray.length} tasks failed${detailed ? `:${pretty(timedoutTasksArray.map(({
    idx,
    taskid
  }) => `Task idx ${idx} (${taskid})`))}` : '\n'}` : '';
  const statusMsg = runningTasksArray.length > 0 ? `${runningTasksArray.length}/${tasksArray.length} tasks running:${pretty(runningTasksArray.map(({
    idx,
    taskid,
    statusName
