...
const fetchRequesterDeals = async (contracts = throwIfMissing(), iexecGatewayURL = throwIfMissing(), requesterAddress = throwIfMissing(), {
  appAddress,
  datasetAddress,
  workerpoolAddress
} = {}) => {
...
const computeTaskId = async (dealid = throwIfMissing(), taskIdx = throwIfMissing()) => {
  try {
    const encodedTypes = ['bytes32', 'uint256'];
    const values = [await bytes32Schema().validate(dealid), await uint256Schema().validate(taskIdx)];
...
const computeTaskIdsArray = async (dealid = throwIfMissing(), firstTaskIdx = throwIfMissing(), botSize = throwIfMissing()) => {
  ...
  const taskids = await Promise.all(tasksIdx.map(idx => computeTaskId(vDealid, idx)));
  return taskids;
};

const show = async (contracts = throwIfMissing(), dealid = throwIfMissing()) => {
  try {
    const vDealid = await bytes32Schema().validate(dealid);
    const {
      chainId
    ...
    const deal = bnifyNestedEthersBn(cleanRPC(await wrapCall(iexecContract.viewDeal(vDealid))));
    ...
    const [{
      workClockTimeRef
    }, timeoutRatio] = await Promise.all([showCategory(contracts, deal.category), getTimeoutRatio(contracts)]);
    const finalTime = deal.startTime.add(timeoutRatio.mul(workClockTimeRef));
    const now = Math.floor(Date.now() / 1000);
    const deadlineReached = now >= finalTime.toNumber();
    const tasks = await computeTaskIdsArray(dealid, deal.botFirst.toString(), deal.botSize.toString());
    const enhancedDeal = {
      dealid: vDealid,
      ...deal,
      finalTime,
...
const getTaskStatus = async (contracts = throwIfMissing(), taskid = throwIfMissing()) => {
  try {
...
const claim = async (contracts = throwIfMissing(), dealid = throwIfMissing()) => {
  const transactions = [];
  const claimed = {};
  ...
  try {
    ...
    if (!deal.deadlineReached) {
      throw Error(`Cannot claim a deal before reaching the final time: ${new Date(1000 * deal.finalTime)}`);
    }
    ...
    await Promise.all(Object.entries(tasks).map(async ([idx, taskid]) => {
    ...
    if (initialized.length > 0) {
      ...
      const processClaims = async () => {
        ...
        const initializedToProcess = initialized.splice(0, maxClaimPerTx.toNumber());
        const taskidToProcess = initializedToProcess.map(({
          taskid
        }) => taskid);
        const tx = await wrapSend(iexecContract.claimArray(taskidToProcess, contracts.txOptions));
        ...
        Object.assign(claimed, ...initializedToProcess.map(e => ({
          [e.idx]: e.taskid
        })));
        await processClaims();
      };
    ...
    if (notInitialized.length > 0) {
      ...
      const processInitAndClaims = async () => {
        ...
        const notInitializedToProcess = notInitialized.splice(0, maxClaimPerTx.toNumber());
        const idxToProcess = notInitializedToProcess.map(({
          idx
        }) => idx);
        const dealidArray = new Array(idxToProcess.length).fill(vDealid);
        const tx = await wrapSend(iexecContract.initializeAndClaimArray(dealidArray, idxToProcess, contracts.txOptions));
        debug(`initializeAndClaimArray ${tx.hash} (${notInitializedToProcess.length} tasks)`);
        await wrapWait(tx.wait(contracts.confirms));
        transactions.push({
          txHash: tx.hash,
          type: 'initializeAndClaimArray'
        });
        Object.assign(claimed, ...notInitializedToProcess.map(e => ({
          [e.idx]: e.taskid
        })));
