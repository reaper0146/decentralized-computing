...
const checkBalance = async (contracts = throwIfMissing(), address = throwIfMissing()) => {
  try {
    ...
    const {
      stake,
      locked
    } = await wrapCall(contracts.checkBalance(vAddress));
    const balance = {
...
const deposit = async (contracts = throwIfMissing(), amount = throwIfMissing()) => {
  try {
    ...
    const {
      nRLC
    } = await checkBalances(contracts, await getAddress(contracts));
    ...
    if (!contracts.isNative) {
      ...
      const tx = await wrapSend(contracts.getRLCContract({
        at: rlcAddress
      }).approveAndCall(iexecAddress, vAmount, NULL_BYTES, contracts.txOptions));
      ...
      const weiAmount = bnToEthersBn(bnNRlcToBnWei(new BN(vAmount))).toHexString();
      const tx = await wrapSend(iexecContract.deposit({
        value: weiAmount,
...
const withdraw = async (contracts = throwIfMissing(), amount = throwIfMissing()) => {
  try {
    const vAmount = await nRlcAmountSchema().validate(amount);
    ...
    const {
      stake
    } = await checkBalance(contracts, await getAddress(contracts));
    if (stake.lt(new BN(vAmount))) throw Error('Withdraw amount exceed account balance');
    const tx = await wrapSend(iexecContract.withdraw(vAmount, contracts.txOptions));
    const txReceipt = await wrapWait(tx.wait(contracts.confirms));
