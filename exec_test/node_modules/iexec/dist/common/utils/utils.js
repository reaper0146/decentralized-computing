...
const isEthUnit = str => ['wei', 'kwei', 'mwei', 'gwei', 'szabo', 'finney', 'ether', 'eth'].includes(str);
...
const bnifyNestedEthersBn = obj => {
  ...
  Object.entries(obj).forEach(e => {
    ...
    } else if (typeof v === 'object' && v._hex) objOut[k] = new BN(v._hex.substring(2), 16);else if (typeof v === 'object') objOut[k] = bnifyNestedEthersBn(v);else objOut[k] = v;
  });
  return objOut;
};
...
const stringifyNestedBn = obj => {
  ...
  Object.entries(obj).forEach(e => {
    const [k, v] = e;
    if (v instanceof BN) objOut[k] = v.toString();else if (typeof v === 'object') {
...
const humanToMultiaddrBuffer = (str, {
  strict = true
} = {}) => {
  let multiaddrBuffer;
...
const cleanRPC = rpcObj => {
  ...
  const cleanObj = keys.reduce((accu, curr) => {
    if (Number.isNaN(parseInt(curr, 10))) {
      ...
      if (Array.isArray(rpcObj[curr]) && !rpcObj[curr].find(e => typeof e === 'object')) {
        value = rpcObj[curr];
...
const getEventFromLogs = (eventName, events, {
  strict = true
} = {}) => {
  let eventFound;
  events.forEach(event => {
...
const encodeTag = tags => {
  ...
  const binString = binaryTags.reduce((acc, curr) => curr ? `1${acc}` : `0${acc}`, '');
...
const findMissingBitsInTag = (tag, requiredTag) => {
  ...
  for (let i = 1; i <= requiredTagBinString.length; i += 1) {
    if (requiredTagBinString.charAt(requiredTagBinString.length - i) === '1' && tagBinString.charAt(tagBinString.length - i) !== '1') {
      missingBits.push(i);
    }
  }
...
const decryptResult = async (encResultsZipBuffer, beneficiaryKey) => {
  ...
  try {
    encryptedResultsKeyArrayBuffer = await zip.file(encKeyFile).async('arraybuffer');
  } catch (error) {
    throw Error(`Missing ${encKeyFile} file in zip input file`);
  }

  const base64encodedEncryptedAesKey = Buffer.from(encryptedResultsKeyArrayBuffer).toString();
  const encryptedAesKeyBuffer = Buffer.from(base64encodedEncryptedAesKey, 'base64');
  ...
  try {
    encryptedZipArrayBuffer = await zip.file(encResultsFile).async('arraybuffer');
  } catch (error) {
  ...
  try {
    ...
    const base64EncodedEncryptedZip = Buffer.from(encryptedZipArrayBuffer).toString();
    const encryptedOutZipBuffer = Buffer.from(base64EncodedEncryptedZip, 'base64');
    const decryptedOutZipBuffer = Buffer.from(aesEcb.decrypt(encryptedOutZipBuffer)); // remove pkcs7 padding

    const padding = decryptedOutZipBuffer[decryptedOutZipBuffer.length - 1];
    const unpaddedDecryptedOutZipBuffer = decryptedOutZipBuffer.slice(0, decryptedOutZipBuffer.length - padding);
